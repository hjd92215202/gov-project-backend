-- 创建数据库（如果不存在）
DROP DATABASE IF EXISTS gov_db;
CREATE DATABASE gov_db DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE gov_db;

-- 1. 确保用户对业务库有全部权限
GRANT ALL PRIVILEGES ON gov_db.* TO 'db_user'@'%';

-- 2. 授予全局的“显示数据库”和“引用”权限，这通常能解决驱动探测元数据失败的问题
GRANT SHOW DATABASES, REFERENCES ON *.* TO 'db_user'@'%';

-- 3. 刷新权限使其生效
FLUSH PRIVILEGES;

-- 彻底清理旧表（按依赖顺序删除）
DROP TABLE IF EXISTS sys_file;
DROP TABLE IF EXISTS biz_project;
DROP TABLE IF EXISTS sys_user_role;
DROP TABLE IF EXISTS sys_user;
DROP TABLE IF EXISTS sys_role;
DROP TABLE IF EXISTS sys_dept;

-- 1. 部门表
CREATE TABLE sys_dept (
    id BIGINT NOT NULL COMMENT 'ID',
    parent_id BIGINT DEFAULT 0 COMMENT '父部门ID(顶级为0)',
    dept_name VARCHAR(50) NOT NULL COMMENT '部门名称',
    leader_id BIGINT DEFAULT NULL COMMENT '部门负责人ID',
    deleted SMALLINT DEFAULT 0 COMMENT '逻辑删除(0正常, 1删除)',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='部门表';

-- 2. 角色表
CREATE TABLE sys_role (
    id BIGINT NOT NULL COMMENT 'ID',
    role_name VARCHAR(50) NOT NULL COMMENT '角色名称',
    role_code VARCHAR(50) NOT NULL COMMENT '角色编码',
    deleted SMALLINT DEFAULT 0 COMMENT '逻辑删除',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='角色表';

-- 3. 用户表
CREATE TABLE sys_user (
    id BIGINT NOT NULL COMMENT 'ID',
    dept_id BIGINT DEFAULT NULL COMMENT '所属部门ID',
    username VARCHAR(50) NOT NULL COMMENT '用户名',
    password VARCHAR(255) NOT NULL COMMENT 'SM3 盐值加密后的密文',
    real_name VARCHAR(50) DEFAULT NULL COMMENT '真实姓名',
    phone VARCHAR(20) DEFAULT NULL COMMENT '手机号',
    status SMALLINT DEFAULT 1 COMMENT '状态：1正常, 0停用',
    deleted SMALLINT DEFAULT 0 COMMENT '逻辑删除',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户表';

-- 4. 用户-角色关联表 (多对多)
CREATE TABLE sys_user_role (
    id BIGINT NOT NULL COMMENT 'ID',
    user_id BIGINT NOT NULL COMMENT '用户ID',
    role_id BIGINT NOT NULL COMMENT '角色ID',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户角色关联表';

-- 5. 工程项目表
CREATE TABLE biz_project (
    id BIGINT NOT NULL COMMENT 'ID',
    project_name VARCHAR(200) NOT NULL COMMENT '工程名称',
    project_code VARCHAR(100) DEFAULT NULL COMMENT '工程编号',
    address VARCHAR(500) DEFAULT NULL COMMENT '工程地址',
    longitude DECIMAL(10, 7) DEFAULT NULL COMMENT '经度',
    latitude DECIMAL(10, 7) DEFAULT NULL COMMENT '纬度',
    leader_name VARCHAR(50) DEFAULT NULL COMMENT '负责人',
    leader_phone VARCHAR(255) DEFAULT NULL COMMENT '负责人电话(SM4加密字段)',
    description TEXT DEFAULT NULL COMMENT '工程描述',
    status INT DEFAULT 0 COMMENT '状态：0待提交, 1审批中, 2已通过, 3被驳回',
    deleted SMALLINT DEFAULT 0 COMMENT '逻辑删除',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='工程项目表';

-- 6. 公共文件表
CREATE TABLE sys_file (
    id BIGINT NOT NULL COMMENT 'ID',
    biz_id BIGINT DEFAULT NULL COMMENT '关联的业务ID',
    file_name VARCHAR(255) DEFAULT NULL COMMENT '原始文件名',
    file_path VARCHAR(500) DEFAULT NULL COMMENT '存储路径',
    file_type VARCHAR(50) DEFAULT NULL COMMENT '文件类型',
    file_size BIGINT DEFAULT NULL COMMENT '文件大小',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='公共文件表';

-- ----------------------------
-- 插入初始数据 (严格保留你的密文)
-- ----------------------------
INSERT INTO sys_role (id, role_name, role_code) VALUES (1, '系统管理员', 'admin');
INSERT INTO sys_dept (id, parent_id, dept_name) VALUES (1, 0, '总经办');
INSERT INTO sys_user (id, dept_id, username, password, real_name, status, deleted)
VALUES (1, 1, 'admin', '66d7edc85a32d756ffef0046a56cf78060276b6beb3f02de7916c01ad54ea6b0', '超级管理员', 1, 0);

ALTER TABLE biz_project
ADD COLUMN province VARCHAR(50) COMMENT '省份',
ADD COLUMN city VARCHAR(50) COMMENT '城市',
ADD COLUMN district VARCHAR(50) COMMENT '区县';

INSERT INTO biz_project (id, project_name, project_code, address, province, city, district, longitude, latitude, leader_name, leader_phone, status)
VALUES
(2, '西安市莲湖区某旧改工程', 'GC-002', '陕西省西安市莲湖区XX路', '陕西省', '西安市', '莲湖区', 108.91000, 34.27000, '李四', '13911112222', 1),
(3, '咸阳市某桥梁项目', 'GC-003', '陕西省咸阳市秦都区YY路', '陕西省', '咸阳市', '秦都区', 108.70000, 34.33000, '王五', '13566667777', 1);